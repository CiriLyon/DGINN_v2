...
server = function(input , output ,session){
  ...
  #--Message d'erreur en cas de fichier qui ne contient pas les colonnes obligatoires.
    ...
    par(xpd = T , mfrow  =c(1,2) ,   oma = c(0,0,0,0) , mgp = c(3,0.3,0))
    par(mar=c(4,10,8,0))
    ...
    h =  barplot(
      ...
      border = NA ,
      axes = F ,
      col = adjustcolor(xt2_col$coul, alpha.f = 0.5),
      horiz = T ,
      las  = 2 ,
      main =  c(
    ...
    xx = barplot(
      ...
      horiz = T ,
      axes = F,
      main = expression(paste("Positively selected sites ratio: Sites number of"~ Gene[i] %/% Gene[i], " length" )) ,
      xlim = c(0, bornSup) ,
      cex.main = 0.9
    )
    ...
    arrows( posX , max(xx) + 3 , posX , max(xx) + 1, code = 2 , col = "magenta" , lwd = 0.001, length = 0.15)
    ...
    PAS <- input$nbGene #= 10 #length(lstGene)+2 # PAS designe le nombre de gènes pas page
...
#########################################fin pdf##############################################################
    ...
    output$sitePos <- renderPlot({
      ...
      PAS = length(lstGene)+2 #10 # PAS designe le nombre de gènes pas page
      par(mfrow = c(PAS,1) ,#c(length(lstGene) , 1) , #c(length(lstGene) , 1), # c(10,1)
          oma = c(1,2,0,0) + 0.1,
          mar = c(20/PAS,8,0,2) + 0.1
      )
